...
fn random_scene() -> hittable_list {
    ...
    world.add(Rc::new(sphere::new(Vec3::new(0.0, -1000.0, 0.0), 1000.0, ground_material.clone())));
    for a in -11..11 {
        for b in -11..11 {
            let choose_mat:f64 = random_f64(&0.0, &1.0);
            let center: Vec3 = Vec3::new(a as f64 + 0.9 * random_f64(&0.0, &1.0), 0.2, b as f64 + 0.9 * random_f64(&0.0, &1.0));
    ...
    world.add(Rc::new(sphere::new(Vec3::new(0.0, 1.0, 0.0), 1.0, material1.clone())));

    let material2 =Rc::new(lambertian::new(&Vec3::new(0.4, 0.2, 0.1)));
    world.add(Rc::new(sphere::new(Vec3::new(-4.0, 1.0, 0.0), 1.0, material2.clone())));

    let material3 = Rc::new(metal::new(&Vec3::new(0.7, 0.6, 0.5), 0.0));
    world.add(Rc::new(sphere::new(Vec3::new(4.0, 1.0, 0.0), 1.0, material3.clone())));
    return world;
}


fn ray_color(r: &Ray, world: &impl hittable, depth: i32) -> Vec3 {
    let mut rec = hit_record {p:Vec3::new(0.0, 0.0, 0.0), normal:Vec3::new(0.0, 0.0, 0.0), mat_ptr:Rc::new(lambertian::new(&Vec3::new(0.0, 0.0, 0.0))), t:0.0, front_face:false,};
