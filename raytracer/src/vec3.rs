...
impl Vec3 {
    ...
    pub fn elemul(left: Self, right: Self) -> Self {
        Self {
            x: left.x * right.x,
            y: left.y * right.y,
            z: left.z * right.z,
    ...
    pub fn unit(&self) -> Self {
        Self {
            x: self.x / Self::length(&self),
            y: self.y / Self::length(&self),
            z: self.z / Self::length(&self),
        }
    }
    pub fn random(min: &f64, max: &f64) ->Self {
         return Vec3::new(random_f64(&min, &max), random_f64(&min, &max), random_f64(&min, &max));
    }
}
